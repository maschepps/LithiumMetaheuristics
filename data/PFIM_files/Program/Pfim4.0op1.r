...
#recover the inital data for all the models
  ...
  #if (graph.only==F) {prots<-lapply(1:nr,function(nr,x) get(x[nr]),x=paste("prot",LETTERS[1:nr],sep=""))}
  #if (graph.only==T) {prots<-lapply(1:nr,function(nr) c(0))}
...
#beta.covaraite:
#theta: value of fixed effects in the stdin.r file
#tab.cov.param
#--------------------
...
for (sequ in 1:lposs){
    ...
    #solve(diag(beta[diag(omega)!=0])%*%omega1%*%diag(beta[diag(omega)!=0]))
...
sommecomplete<-cbind(somme,sommecomplete)
 first.line<-c("# Project: ",project,                        '        Date: ',date() ) }
    ...
    return(list(form,somme,subjects,doses,prots,se,cv,det,crit,pfixe,pp,xn,p,sh))  
}
...
#Calcul du nombre de sujet necessaires pour avoir une puissance donnee%
#given.power
#calcul de la SE si je me mets sous H1
NSE<-c()
for (i in 1:length(beta.cov)){
...
cat("\n","\n","\n") }
  ...
  cat('OK AF option 1','\n') 
	if (covariate.model==T)
	   {return(list(doses=dose,prots=prots,subjects=subjects,mfisher=mfisher,determinant=determinant,crit=crit,se=se,cv=cv,summary.ci=summary.ci,summary.power_nni=summary.power_nni,summary.ci_eq=summary.ci_eq,summary.power_nni_eq=summary.power_nni_eq,EigenValues=EigenValues,corr.matrix=corr.mat))}
  if (covariate.model==F)
  	 {if (FIM!="B") return(list(doses=dose,prots=prots,subjects=subjects,mfisher=mfisher,determinant=determinant,crit=crit,se=se,cv=cv,EigenValues=EigenValues,corr.matrix=corr.mat))
      if (FIM=="B") return(list(doses=dose,prots=prots,subjects=subjects,mfisher=mfisher,determinant=determinant,crit=crit,se=se,cv=cv,Shrinkage=sh,EigenValues=EigenValues,corr.matrix=corr.mat))
     }
